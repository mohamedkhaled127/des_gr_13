...
inversePermutation(bitSwap,encr_decr_Binary);
...
/*----------------------------------------------------------------------------------------------*/
printCypherOutput(encr_decr_Binary);
printEncrToHex(encr_decr_Binary);
//printf("\n","encryption finished");


initialPermutation(encr_decr_Binary,outputSliced);  //initial permutation
	        	/*for(int ii=0;ii<64;ii++){
	        		printf("%d",outputSliced[ii]);
...
printPlainOutput(encr_decr_Binary);
